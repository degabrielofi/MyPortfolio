    ...
    <>
      <header
        style={{
          ...
          transition: 'all .5s',
          ...(scrolled || menuOpen ? { background: 'color-mix(in srgb, var(--color-bg) 92%, transparent)', backdropFilter: 'blur(20px) saturate(1.6)', borderBottom: '1px solid var(--color-bdr)' } : {}),
        }}
      >
        <div style={{ width: '100%', maxWidth: 1060, paddingLeft: 'max(20px, 6vw)', paddingRight: 'max(20px, 6vw)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <div
        ...
        style={{
          ...
          borderBottom: menuOpen ? '1px solid var(--color-bdr)' : 'none',
          padding: menuOpen ? '20px max(20px, 6vw) 24px' : '0 max(20px, 6vw)',
      ...
      >
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 20 }}>
          {links.map(({ href, label }) => (
            <a key={href} href={href} onClick={() => setMenuOpen(false)} className="font-display" style={{ fontSize: 13, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase' as const, color: 'var(--color-tx3)', padding: '10px 0', borderBottom: '1px solid var(--color-bdr)', transition: 'color .3s' }}>
              {label}
            </a>
          ))}
        </nav>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          {LOCALES.map(({ code, label }, i) => (
            <span key={code} style={{ display: 'flex', alignItems: 'center' }}>
              <button onClick={() => { router.replace(pathname, { locale: code }); setMenuOpen(false); }} className="font-mono" style={{ fontSize: 11, fontWeight: 500, padding: '2px 4px', border: 'none', background: 'none', cursor: 'pointer', color: locale === code ? 'var(--color-ac)' : 'var(--color-tx4)', transition: 'color .3s' }}>
                {label}
              </button>
              {i < LOCALES.length - 1 && <span style={{ color: 'var(--color-tx4)', fontSize: 10, margin: '0 2px' }}>·</span>}
            </span>
